...
public class ArduinoHM10 : MonoBehaviour
{
...
//	public GameObject PanelMiddle;
		...
		if (_timeout > 0f)
		{
			...
			if (_timeout <= 0f)
			{
				...
				case States.Scan:
					...
					Debug.Log("Scanning for HM10 devices...");
				...
				case States.Connect:
					...
					BluetoothLEHardwareInterface.SubscribeCharacteristicWithDeviceAddress (_hm10, ServiceUUID, Characteristic, null, (address, characteristicUUID, bytes) => {
						...
						BluetoothStatus.text = "Received Serial: " + data;
						
						if( data == "Up")
							//GameObject.FindGameObjectsWithTag("MainCamera").GetComponent<SpeedController>().increaseSpeed();
							sc.GetComponent<SpeedController>().increaseSpeedBle();
						if( data == "Down")
							//GameObject.FindGameObjectsWithTag("MainCamera").GetComponent<SpeedController>().decreaseSpeed();
							sc.GetComponent<SpeedController>().decreaseSpeedBle();
						if( data.IndexOf( "peed") > 0 )
							sc.GetComponent<SpeedController>().setSpeed(data.Split(':')[1]);
						if( data.IndexOf( "evel") > 0 )
							sc.GetComponent<SpeedController>().setIntensity(data.Split(':')[1]);
	...
	{
		Debug.Log("Sending [" + value + "] over ble!");
